...
\begin{figure*}[ht]
  ...
  Colors \coolwarmbox\space in panels (a) and (b) indicate $\log_{10}$-scaled relative risk values, ranging from low {\color{TUblue}($-2$)} to high {\color{TUred}($2$)}.
  (a) Zoomed-in view of the network showing estimated segment and junction risks.
  (b) Detailed view of junction 2482 with assigned crashes (black dots).
  (c) Street-level image of junction 2482~\citep{streetview2024}, providing visual context for the observed risk.
  }
