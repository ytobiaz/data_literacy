    ...
    *,
    ...
    def _aggregate(df, id_col):
        ...
        if metric in df.columns:
            agg_spec[metric] = (metric, "first")
        out = df.groupby(id_col, as_index=False).agg(**agg_spec)
        return gpd.GeoDataFrame(out, geometry="geometry", crs=df.crs)

    seg = _aggregate(_ensure_geo(top_segments), "counter_name")
    jn = _aggregate(_ensure_geo(top_junctions), "node_id")

    seg[metric] = seg[metric].replace([np.inf, -np.inf], np.nan).fillna(0.0)
    jn[metric] = jn[metric].replace([np.inf, -np.inf], np.nan).fillna(0.0)

    vals = np.concatenate([
        seg.loc[seg[metric] > 0, metric].to_numpy(),
        jn.loc[jn[metric] > 0, metric].to_numpy(),
    ])
    vabs = float(np.nanpercentile(np.abs(np.log10(vals)), clip_percentile)) if vals.size else 1.0
    vabs = max(vabs, 0.1)
    floor = 10 ** (-vabs)

    seg["_metric"] = seg[metric].clip(lower=floor)
    seg["_log_rr"] = np.log10(seg["_metric"])
    seg_wgs = seg.to_crs(epsg=4326)

    jn["_metric"] = jn[metric].clip(lower=floor)
    jn["_log_rr"] = np.log10(jn["_metric"])
    jn_wgs = jn.to_crs(epsg=4326)
    ...
    cmap.caption = f"log10({metric})"

    m = folium.Map(location=center, zoom_start=zoom, tiles="CartoDB positron")

    # Segments
    seg_fields = ["counter_name", metric]
    seg_aliases = ["Segment", metric]
    if "exposure" in seg_wgs.columns:
        seg_fields.append("exposure")
        seg_aliases.append("Exposure")
    if "total_accidents" in seg_wgs.columns:
    ...
    folium.GeoJson(
        seg_wgs[seg_fields + ["_log_rr", "geometry"]],
        name=f"Segments ({metric})",
        style_function=_seg_style,
        tooltip=folium.GeoJsonTooltip(fields=seg_fields, aliases=seg_aliases, localize=True),
    ).add_to(m)
    ...
    for _, r in jn_wgs.iterrows():
        ...
        folium.CircleMarker(
            ...
            weight=1,
            tooltip=(
                f"node_id={int(r.node_id)} | {metric}={float(r[metric]):.2f}"
                f"{exposure_txt}{accidents_txt}"
            ),
        ).add_to(m)

    # Accidents (year + severity layers)
    ...
    acc = acc.dropna(subset=[lon_col, lat_col])
    acc_gdf = gpd.GeoDataFrame(
        acc,
        geometry=gpd.points_from_xy(acc[lon_col], acc[lat_col]),
        crs="EPSG:4326",
    )

    years = sorted(acc_gdf[year_col].dropna().unique().tolist())
    severities = sorted(acc_gdf[severity_col].dropna().unique().tolist())

    for y in years:
        for s in severities:
            subset = acc_gdf[(acc_gdf[year_col] == y) & (acc_gdf[severity_col] == s)]
            if subset.empty:
                continue
            layer = folium.FeatureGroup(name=f"Accidents {y} | severity {s}", show=False)
            for _, r in subset.iterrows():
                ...
                folium.CircleMarker(
                    ...
                    fill_opacity=0.6,
                    weight=0,
                    tooltip=f"{year_col}={y} | {severity_col}={s}",
                ).add_to(layer)
            layer.add_to(m)
