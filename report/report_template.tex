...
\]
and the raw relative risk $r^{\text{raw}}_i$ is $A_i/\widehat{A}_i$.
...
Concretely, we assume a true relative risk  $r^{\text{true}}_i$ such that the observed count $A_i$ follows a Poisson model with 
\[
A_i \mid r^{\text{true}}_i \sim \text{Poisson}(\widehat{A}_i\,r^{\text{true}}_i).
\]
The Poisson distribution is natural for nonnegative event counts over a fixed time period under a baseline rate, and it yields $\mathbb{E}[A_i]=\widehat A_i$ when $r^{\text{true}}_i=1$. To allow heterogeneity in 
...
\[
r^{\text{true}}_i \mid A_i,\widehat{A}_i \sim \text{Gamma}(A_i+\alpha,\;\widehat{A}_i+\alpha),
\]
...
\[
\widehat{r}_i=\mathbb{E}[r^{\text{true}}_i\mid A_i,\widehat A_i]=\frac{A_i+\alpha}{\widehat A_i+\alpha}
\]
as the smoothed relative risk. For small $\widehat A_i$, $r_i$ is pulled toward 1, while for large $\widehat A_i$ it approaches the raw ratio $A_i/\widehat A_i$. Uncertainty is summarized by $(1-\delta=0.95)$ 
...
\begin{figure*}[ht]
  ...
  Panel (b) provides a closer view of junction 2482, displaying the 22 crashes assigned to this junction. 
...
A section of the resulting risk map is shown in \cref{fig:junction_details} (a).
...
The detailed view in \cref{fig:junction_details} (b) shows all crashes assigned to this junction, the majority of which are located along the lane of Hermann-Hesse-Straße turning into Heinrich-Mann-Straße.
...
To contextualize these findings, a street-level image of the junction from Google Street View \cite{streetview2024} in \cref{fig:junction_details} (c) shows the car lanes crossing the bicycle lane at this location.
