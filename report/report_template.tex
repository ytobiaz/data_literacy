...
\begin{table}[ht]
\centering
\caption{\textbf{Comparative analysis of path length increase ($\Delta_L$) and risk reduction ($\Delta_R$) across varying detour budgets ($\varepsilon$) and junction weights ($\eta$).}
Results aggregated over all origin-destination pairs; medians (interquartile ranges) shown. $\Delta_L$ and $\Delta_R$ are relative scales; $P(\Delta_R > 0)$ is a percentage.}
