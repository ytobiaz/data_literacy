...
\begin{figure*}[t]
  ...
  Panels (a--c) show an example year (2021) and are zoomed in for readability; see \cref{sec:methods} for formal definitions and notation.
  (a): Police-recorded bicycle crashes ({\color{TUorange}points}) and street segments with measured cyclist exposure ({\color{TUgray}lines}, used as the base network in all panels).
  (b): Yearly segment-level relative crash risk; high-risk segments in {\color{TUred}red} correspond to the top 90th percentile of $r_{s,y}$ in the displayed year. {\color{TUdark}Circles} mark junctions (degree $\ge 3$), for which we also estimate junction-level relative risk from exposure aggregated over incident segments.
...
For each network entity $i$, either a street segment or a junction, and month $t$, let $A_{i,t}$ denote the number of police-recorded bicycle crashes and $E_{i,t}$ the measured cyclist exposure. We assume that, within a given month, crash incidence is proportional to exposure under a ``no special risk'' baseline. This yields the expected number of crashes
\[
\widehat{A}_{i,t}
...
To capture the concentration of crashes near intersections, we estimate junction-level relative risk analogously. A junction is defined as any network node with degree $\ge 3$, and crashes within a fixed radius of its centroid are assigned to that junction. Because a traversal typically contributes exposure to two incident segments, entering and exiting, we approximate junction exposure by a half-sum of incident segment exposures,
\begin{equation}
\label{eq:half-sum}
E_v=\tfrac{1}{2}\sum_{s\in\mathcal{I}(v)} E_s,
\end{equation}
which avoids double-counting. Aggregating exposure over incident links is common when turning movements are unavailable~\citep{hakkert2002uses,WANG2020105838}. Junction crashes and exposures enter the same pooled Empirical Bayes formulation as segments.
