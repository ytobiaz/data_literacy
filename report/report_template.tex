...
\]
and the relative crash reduction
\[
\Delta_R=\frac{R(P_{\text{dist}})-R(P_{\text{safe}})}{R(P_{\text{dist}})}.
\]
Pairs with $R(P_{\text{dist}})=0$ are excluded from $\Delta_R$ due to the undefined denominator.
We also report the avoided crashes
\[
...
\begin{table}[ht]
\centering
\caption{Trade-off between path length increase and safety improvements for varying detour budgets ($\varepsilon$) and junction weights ($\eta$).
Values are aggregated over all origin--destination pairs and reported as medians.
$\Delta_L$ denotes relative path length increase,
$\Delta_R$ the relative reduction in expected crashes with respect to the shortest-path baseline,
and $\Delta_C$ avoided expected crashes (per 100k trips).}
\label{tab:routing_tradeoff}
\begin{tabular}{ccccc}
\toprule
$\eta$ & $\varepsilon$ & $\Delta_L$ & $\Delta_R$ & $\Delta_C$ \\
\midrule
0.0 & 0.05 & 0.007 & 0.187 & 32.0 \\
    & 0.10 & 0.013 & 0.239 & 43.8 \\
    & 0.20 & 0.034 & 0.296 & 61.9 \\
\addlinespace
0.5 & 0.05 & 0.007 & 0.155 & 35.8 \\
    & 0.10 & 0.013 & 0.182 & 50.5 \\
    & 0.20 & 0.036 & 0.254 & 70.6 \\
\addlinespace
1.0 & 0.05 & 0.007 & 0.136 & 38.7 \\
    & 0.10 & 0.015 & 0.174 & 60.6 \\
    & 0.20 & 0.041 & 0.235 & 82.7 \\
\bottomrule
\end{tabular}
\end{table}
