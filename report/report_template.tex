...
and $\Delta_C$ the number of avoided expected crashes per 100{,}000 trips, reported as rounded integer counts.}
...
While the interquartile ranges (IQR) for safety gains ($\Delta_R, \Delta_C$) remain relatively stable across budgets, the IQR for path length ($\Delta_L$) expands significantly from $0.022$ to $0.200$, indicating that the distance cost required to achieve safety improvements is highly sensitive to the specific layout of the route.
Furthermore, the likelihood of finding a safer route ($\Delta_R > 0$) remains high across all budgets, increasing from $76.7\%$ to $94.8\%$ as the search space expands.
...
Our approach faces two main limitations. First, official crash data capture only personal injury incidents and suffer from under-reporting, systematically underestimating absolute risk. Second, Strava data skew toward fast rides on main roads, underrepresenting trips on residential streets. Where Strava inflates exposure, the calculated risk is too low; where it understimates exposure, sparse data yield inflated risk estimates. Empirical Bayes smoothing partially mitigates this by shrinking unstable low-exposure estimates toward the citywide baseline.
Another issue is, that for calculating the junction exposure we assume uniform flow and ignore turning movements. This misallocates exposure where one main road dominates side streets. To improve the accuracy of risk estimates, cities like Berlin could expand their automated counting infrastructure. This could offer ground truth data that, through deep learning methods, can be used to improve interpolation of bike exposure from sparse data \citep{kaiser2025spatiotemporalgraphneuralnetwork}. 
